...
class Captionpix_Defaults extends Captionpix_Admin {
	...
	function page_content() {
 		$title = $this->admin_heading('Defaults','dashicons '.CAPTIONPIX_ICON);				
		$this->print_admin_form_with_sidebar($title,__CLASS__, array_keys($this->tips)); 
	} 

	function load_page() {
 		$message =  isset($_POST['options_update']) ? $this->save() : '';	
		$options = Captionpix_Options::get_options($this->use_cache());	
		$callback_params = array ('options' => $options, 'message' => $message);
		$this->add_meta_box('intro', 'Intro', 'intro_panel', $callback_params);
		$this->add_meta_box('general', 'Display Defaults', 'general_panel', $callback_params);
...
INTRO_PANEL;
	...
	function general_panel($post, $metabox) {		
		...
		$this->print_form_field('theme', $options['theme'], 'select', $theme_list);
		$this->print_text_field('width', $options['width'], array('size' => 4, 'maxlength' => 4,'suffix' => 'pixels'));
		$this->print_text_field('marginside', $options['marginside'], array('size' => 4, 'maxlength' => 3,'suffix' => 'pixels'));
		$this->print_text_field('margintop', $options['margintop'], array('size' => 4, 'maxlength' => 3,'suffix' => 'pixels'));
		$this->print_text_field('marginbottom', $options['marginbottom'], array('size' => 4, 'maxlength' => 3,'suffix' => 'pixels'));
		$this->print_form_field('align', $options['align'], 'select', array('none','center','left','right'));
	}
