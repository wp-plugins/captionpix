...
abstract class Captionpix_Admin {
   ...
   private $messages = array();
	...
	function __construct($version, $path, $parent_slug, $slug = '') {
		...
		$this->tooltips = new Captionpix_Tooltip($this->tips);
		$this->init();
	}
	...
	function register_tooltip_styles() {
		Captionpix_Utils::register_tooltip_styles();	
	}	

	function enqueue_tooltip_styles() {
		Captionpix_Utils::enqueue_tooltip_styles();
	}	
   ...
   function enqueue_metabox_scripts() {
	...
	function form_field($id, $name, $label, $value, $type, $options = array(), $args = array(), $wrap = false) {
		...
		$label_args = (is_array($args) && array_key_exists('label_args', $args)) ? $args['label_args'] : false;
 		return Captionpix_Utils::form_field($id, $name, $this->tooltips->tip($label, $label_args), $value, $type, $options, $args, $wrap);
 	}	
	...
	function print_text_field($fld, $value, $args = array()) {
 	...
 	function get_meta_form_data($metakey, $prefix, $values ) {
      $content = array();
		if (($post_id = Captionpix_Utils::get_post_id())
		&& ($meta = Captionpix_Utils::get_meta($post_id, $metakey)))
			$values = Captionpix_Options::validate_options($values, $meta);	
   ...
   function get_newsfeeds() {
      return apply_filters('captionpix_newsfeeds', array(CAPTIONPIX_NEWS));
   }

 	function news_panel($post,$metabox){	
		Captionpix_Feed_Widget::display_feeds($this->get_newsfeeds());
	}
	...
	function admin_heading($title = '', $icon_class = '') {
		...
		if (empty($icon_class)) $icon_class = CAPTIONPIX_ICON;
   ...
   function print_admin_form ($title, $referer = false, $keys = false, $enctype = false, $preamble = false) {
	...
	function display_metabox($tabs) {
      ...
      printf('<div class="captionpix-metabox"><ul class="captionpix-metabox-tabs">%1$s</ul><div class="metabox-content">%2$s</div><input type="hidden" id="tabselect" name="tabselect" value="%3$s" /></div>', $labels, $contents, $tab);
   }
